...
def load_config():
    ...
    config['file_name_xls'] = config['syn_exp_dir'] + '_nn_scores.xls'
    config['syn_exp_dir'] = os.path.join('.', 'Experiments', config['syn_exp_dir'])

    print("Loading experiment %s from file: %s" % (args_opt.config, exp_config_file))
    print("Generated logs, snapshots, and model files will be stored on %s" % (config['syn_exp_dir']))
...
if __name__ == "__main__":
    ...
    ######## Excel file ########
    file_name_xls = config['file_name_xls']
    ...
    for target_name in test_opt['target_names']:
        data_dir = os.path.join(test_opt['root_dir'], test_opt['data_dir'])
        ...
        for video_name in video_names:
            train_video_name = video_name.replace(target_name, train_opt['target_names'][0])
            real_video_dir = os.path.join(data_dir, train_video_name)
            # real_video_dir = os.path.join(data_dir, video_name)

            syn_videos_dir_name = target_name + '_syn_NN'
            syn_videos_dir = os.path.join(config['syn_exp_dir'], syn_videos_dir_name)
            syn_video_dir = os.path.join(syn_videos_dir, video_name)

            print('\n')
            print('The results of NN:')
