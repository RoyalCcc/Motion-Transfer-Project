...
parser.add_argument('--img_root', type=str,
                    default='../Motion-Datasets/subject5/test')
...
img_dir = os.path.join(args.img_root, 'test_img')
img_names = sorted(os.listdir(img_dir))
img_names = [name for name in img_names if name != '.DS_Store']

result_dir = os.path.join(args.img_root, 'test_poseImg')
if not os.path.isdir(result_dir):
    os.makedirs(result_dir)

bbx_dir = os.path.join(args.img_root, 'test_bbx')
if not os.path.isdir(bbx_dir):
    os.makedirs(bbx_dir)

failed_dir = os.path.join(args.img_root, 'failed_img')
