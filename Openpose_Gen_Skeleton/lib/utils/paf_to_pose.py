            ...
            if is_added:
                ...
                if score > max_score:
                    ...
                    for key, body_part in human.body_parts.items():
                        if body_part.x < x_min:
                            x_min = body_part.x
                        if body_part.x > x_max:
                            x_max = body_part.x
                        if body_part.y < y_min:
                            y_min = body_part.y
                        if body_part.y > y_max:
                            y_max = body_part.y
                    result_w = x_max - x_min
                    result_h = y_max - y_min
                # humans.append(human)

    bbx_info = [result_w, x_min, x_max, result_h, y_min, y_max]

    return result_human, bbx_info
